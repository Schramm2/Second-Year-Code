...
import static clubSimulation.ClubSimulation.clubGrid;
...
public class ClubGrid {
	...
	public synchronized GridBlock whereEntrance() {
		return entrance;
	} //Synchronized to avoid congestion on blocks as grid is shared
	...
	public synchronized GridBlock enterClub(PeopleLocation myLocation) throws InterruptedException  { //Synchronized to avoid two patrons trying to enter at the same time
	...
	public synchronized GridBlock move(GridBlock currentBlock,int step_x, int step_y,PeopleLocation myLocation) throws InterruptedException {  //Synchronized to make sure the patrons dont move onto the same block
	...
	public void leaveClub(GridBlock currentBlock,PeopleLocation myLocation)   {
			...
			synchronized (entrance){
				entrance.notifyAll();//Telling the patrons that the entrance is now available
			}
	...
	public synchronized GridBlock getExit() {
		return exit;
	}//Synchronized to avoid congestion on the exit block
