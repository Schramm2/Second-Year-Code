...
public class ClubSimulation {
	...
	private static int minWait=1100; //for the fastest cutomer
	public static AtomicBoolean startPressed = new AtomicBoolean(false);
	public static AtomicBoolean pausePressed = new AtomicBoolean(false);
	protected static CountDownLatch latch = new CountDownLatch(1);;
	private static AtomicBoolean entranceOcc = new AtomicBoolean(false);
	...
	public static void setupGUI(int frameX,int frameY,int [] exits) {
		...
		startB.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent e)  {
				...
				startPressed.set(true);//Setting the start button boolean to true
			...
			pauseB.addActionListener(new ActionListener() {
		      public void actionPerformed(ActionEvent e) {
				  if (pausePressed.get() == false){
					  pausePressed.set(true);//Setting pause button boolean to true
				  }else {
					  pausePressed.set(false);//Setting pause button boolean to false
				  }

				  System.out.println("Pause Pressed and is: "+pausePressed);
