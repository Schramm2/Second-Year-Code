...
import static clubSimulation.ClubSimulation.*;
...
public class Clubgoer extends Thread {
	...
	private void checkPause() throws InterruptedException {
		...
		synchronized (pausePressed){
			while (pausePressed.get()==true){
				pausePressed.wait();
			}
			pausePressed.notify();
		}
	...
	private void startSim() throws InterruptedException {
		synchronized (startPressed){
			while(!startPressed.get()){
			}
			latch.countDown();
		}
	...
	private void checkCapacity() throws InterruptedException{//Method to check the capacity of the club before the patron enters
		synchronized (tallys){
			if(tallys.overCapacity()){
				tallys.personArrivedAndWaiting();
				while(tallys.overCapacity()){
					try{
						tallys.wait();//Telling the patron to wait until there is a space
					}catch (InterruptedException e){

					}
				}
				tallys.personArrivedAndWaitingLeft();
	...
	public void run() {
		try {
			startSim();
			latch.await();//Telling them to wait for the start button to be pressed
			checkPause();
			sleep(movingSpeed*(rand.nextInt(100)+1)); //arriving takes a while
			checkPause();
			myLocation.setArrived();
			System.out.println("Thread "+ this.ID + " arrived at club");
			 //output for checking
			checkPause(); //check whether have been asked to pause


			checkCapacity();//Checking capacity
			...
			while (inRoom) {	
				...
				if((!thirsty)&&(!wantToLeave)) {
					if (rand.nextInt(100) >95) 
						thirsty = true; //thirsty sometimes
				...
				if (wantToLeave) {	 //leaving overides thirsty	
					...
					if (currentBlock.isExit() ) {
						...
						synchronized (tallys){
							...
							tallys.notifyAll();//Notifying the patrons that a space is available in the club
						}
						leave();
